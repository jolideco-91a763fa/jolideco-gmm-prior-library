...
VO_URL = "http://{0}/gleam_postage/q/siap.xml?FORMAT=ALL&VERB=2" "&NTERSECT=OVERLAPS&"
...
def download_file(
    url, ra, dec, ang_size, freq, download_dir, file_name_func=None, clobber=True
):
    """ """
...
def vo_get(
    ...
    **kwargs
    ...
    """
    if ang_size > 5.0:
        raise GleamClientException("Angular size %.1f > 5.0 (degrees)" % ang_size)

    if download_dir and (not os.path.exists(download_dir)):
        raise GleamClientException("Invalid download dir: {0}".format(download_dir))

    if not proj_opt in PROJ_OPTS:
        raise GleamClientException(
            "Invalid projection: '{0}'."
            " Should be one of {1}".format(proj_opt, PROJ_OPTS)
        )
